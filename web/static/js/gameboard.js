...
$('.labels.horizontal').each(function (i, item) {
  ...
  GoGame = (function() {
    ...
    */
    ...
    GoGame.prototype.handleClick = function(x, y, is_try) {
      var c;
      if (!this.scoring) {
        ...
        if (is_try == false) {
          ...
          move = String.fromCharCode(x+'A'.charCodeAt(0)) + String.fromCharCode(y+'A'.charCodeAt(0))
          $('#current_try_move').attr('value', move);
        }
      }
      if (this.scoring) {
        c = this.board.cells[x][y];
  ...
  G = new GoGame('#board19');

  if (game_moves != '') {
    moves = game_moves.split(',')
    for (i=0; i<moves.length; i++) {
      x = moves[i][0].charCodeAt(0) - 'A'.charCodeAt(0)
      y = moves[i][1].charCodeAt(0) - 'A'.charCodeAt(0)
      G.handleClick(x, y, false)
    }
  }
